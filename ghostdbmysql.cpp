...
vector<CDBBan *> MySQLBanList( void *conn, string *error, uint32_t botid, string server )
{
	...
	//string Query = "SELECT name, ip, DATE(date), gamename, admin, reason, expiredate FROM bans WHERE warn='0' AND server='" + EscServer + "' ORDER BY name"; //This is the original
	//New (does this make !sc useless?): 
	string Query = "SELECT name, ip, DATE(date), gamename, admin, reason, expiredate FROM bans WHERE warn='0' AND (server='" + EscServer + "' OR server='') ORDER BY name";
	if( mysql_real_query( (MYSQL *)conn, Query.c_str( ), Query.size( ) ) != 0 )
		*error = mysql_error( (MYSQL *)conn );
	else
	{
...
//					double Score = UTIL_ToDouble( Row[21] );
					...
					if(false)
					{ // we no longer need this, we have the 'rating' column now
						uint32_t botid = 0;
						CDBScoreSummary *DotAScoreSummary = ScoreCheck((MYSQL *)conn, error, botid, name);
						if (DotAScoreSummary)
						{
...
CCallableDotAPlayerSummaryCheckNew *CGHostDBMySQL :: ThreadedDotAPlayerSummaryCheckNew( string servername, string name, string mingames, string gamestate )
{
	...
	CCallableDotAPlayerSummaryCheckNew *Callable = new CMySQLCallableDotAPlayerSummaryCheckNew( servername, name, UTIL_ToString(GetMinGames()), gamestate, Connection, m_BotID, m_Server, m_Database, m_User, m_Password, m_Port );
	CreateThread( Callable );
	m_OutstandingCallables++;
	return Callable;
}
